...
const boxMaterial = new THREE.ShaderMaterial({
  ...
  void main()	{
    ...
    // result = vec4(position.x, 4.0*sin(position.z/4.0) + position.y, position.z, 0.0);
    result = vec4(position.x, 4.0*sin(position.z/4.0 + u_time) + position.y, position.z, 0.8);
  ...
  void main() {
    ...
    gl_FragColor = vec4(abs(sin(u_time)), 1.0, 0.1, 1.0);
  }
  `,
});
const boxMesh = new THREE.Mesh(boxGeometry, boxMaterial);
